...
class Task(models.Model):
    ...
    source_adres = models.CharField(max_length=250, null=False)  #Адрес файла, пока только telegram fileid, будут ссылки
    additional_price = models.DecimalField(max_digits=5, decimal_places=3, null=True, default=0)  #Плата за использование доп. сервисов. Пока не используеся
    ...
    send_instastories_proposal = models.BooleanField(default=False)  #Предлагать подписаться на истории (Пока не используется)
    FC_max_results = models.IntegerField(default=0, blank=True, null=True)  #Сколько результатов добавить в PDF
    FC_tolerance = models.DecimalField(max_digits=5, decimal_places=3, default=99)  #Какой коэфициент FC считать совпадением
    PY_max_results = models.IntegerField(default=0, blank=True, null=True)  # Сколько результатов добавить в PDF
    PY_tolerance = models.DecimalField(max_digits=5, decimal_places=3, default=99)  #Какой коэфициент PY считать совпадением
    # ПОЛЯ ЗАПОЛНЯЕМЫЕ БОТОМ
    ...
    log = models.TextField(blank=True, default=f'{timezone.now().strftime("%d-%m-%Y %H:%M:%S")} поиск создан<br />')  ##Сюда добавляется строки походу раоты парсера
